...
!!! Abstract "Outline"
    ...
    - [x] 基本概念：[进程执行、调度队列与上下文切换](#_1)
    - [x] 调度准则：[CPU 利用率、吞吐量、周转时间、等待时间与响应时间](#_2)
    - [x] 调度策略：[FCFS、SJF、RR 与优先级调度](#_3)
    - [x] 多处理器调度：[调度方法、多核处理器、负载均衡与处理器亲和性](#_4)
...
每个进程在 Kernel Space 都会有类似下面这张图一样的内核栈，保存着线程信息等等需要保存的值，其中 `pt_regs` 结构体保存着完整的寄存器上下文，用于模式切换，当发生异常、中断和系统调用的时候保存到其中，其位于内核栈的顶部；`thread_info` 结构体仅仅保存一些最重要的寄存器等信息，用于上下文切换，这部分为 `cpu_context` 结构体，位于内核栈的底部：
...
<img class="center-picture" src="assets/topic2/Schedule-12.webp" alt="drawing" width="500" />

值得注意的是，当两个用户态的进程相互切换的时候，实际发生了两次模式切换和一次上下文切换，模式切换完成内核态和用户态的切换，上下文切换完成对应进程内核态的切换，在内核态切换的时候，操作系统内核就拥有了对应进程的所有信息，包括 `pt_regs` 结构体，再完成模式切换，保证正确。

这就可以解释为什么 `fork` 可以返回两个值了：简单来讲，就是因为 `fork` **复制了一份父进程的上下文**，对于父进程，这就是系统调用，返回值保存在对应的 `pt_regs` 结构体中，在系统调用返回的时候恢复，从而返回一个值；对于子进程，这部分的值也通过 `pt_regs` 结构体保存，在 `do_fork` 的时候设置返回值为 0，在恢复的时候返回。

操作系统的设计强调机制/Machanism 与策略/Policy 分离，这里谈到的机制就是分配器、队列与上下文切换，策略就是调度策略。上文基本已经阐述清楚了 CPU 调度的机制，下面我们将以几个调度策略来考虑如何设计优秀的调度策略。

## 调度准则

宏观来讲，并没有一个万能的调度准则，很可能几个调度准则之间是冲突的，调度准则的制定与使用需要根据具体场景来考虑。

- 最大化 CPU 利用率/CPU Utilization：其定义为 CPU 不处于空闲时间的比例；
- 最大化吞吐量/Throughput：吞吐量定义为一个单位时间内完成的工作的数量，对于长进程，吞吐量可能是几秒钟一个进程，对于段锦程，可能是一秒钟几十个进程；
- 最小化周转时间/Turnaround Time：周转时间定义为进程从提交到结束的时间，这个时间包括在就绪队列之中等待的时间、在 CPU 上执行的时间以及 I/O 执行的时间；
- 最小化等待时间/Waiting Time：等待时间定义为进程在就绪队列之中等待的时间；
- 最小化响应时间/Response Time：响应时间定义为进程从提交到第一次被响应的时间。
...
简单来说，我们可以将就绪队列分为多个子队列，只需要实现多个队列之间的调度和队列内部的调度就可以。比如可以为每一个优先级设置单独的队列，优先级调度使用在队列之间进行调度，只不过是将进程调度到最高优先级队列之中，而高优先级队列内部可以使用轮询调度，低优先级队列内部可以使用 FCFS 调度。

多级队列调度可以根据进程类型将进程分为不同的队列，比如台前交互式进程和后台批处理进程之间的划分，台前队列使用轮询调度，后台队列使用 FCFS 调度。

队列之间也应该有调度，一般使用固定优先级抢占调度，比如实时队列可以比后台队列有绝对的优先级。但是一般来讲当进程进入到系统的时候，就会被永久分配到一个队列之中，这种方法开销比较低，但是不太灵活。

### 7. Multi-Level Feedback Queue Scheduling

多级反馈队列调度/Multi-Level Feedback Queue Scheduling 是多级队列调度的一个重要扩展，它通过允许进程在不同优先级的队列之间动态迁移，实现了对系统资源的更灵活和高效的利用。这种算法的核心思想是根据进程在运行过程中的行为动态地调整其优先级。具体来说，它试图根据进程的 CPU 突发时间来区分不同类型的进程。例如，频繁进行 I/O 操作或与用户交互的进程，其 CPU 突发时间通常较短，应被置于高优先级队列；而需要大量计算的 CPU 密集型进程，其 CPU 突发时间较长，则应被分配到低优先级队列。此外，为了防止低优先级队列中的进程长时间得不到执行，该算法也允许等待时间过长的进程被提升到更高的优先级队列。

为了更好地理解其工作原理，我们可以考虑一个具体实现。假设系统中有三个队列：`Q0`，`Q1` 和 `Q2`，优先级从高到低。一个新创建的进程首先进入最高优先级的队列 `Q0`，并被分配一个较小的时间片，例如 8 毫秒。如果该进程在时间片用完之前就主动放弃 CPU，那么系统会认为它可能是一个 I/O 密集型或交互式进程。因此，当它下次就绪时，它会继续留在 `Q0` 中，以便在需要 CPU 时能迅速得到响应。相反，如果进程用完了整个 8 毫秒的时间片，这表明它可能是一个 CPU 密集型进程，于是它会被降级到次优先级的队列 `Q1`。在 `Q1` 中，它会得到一个更长的时间片，比如 16 毫秒。如果它再次用完了 `Q1` 的整个时间片，它就会被进一步降级到最低优先级的队列 `Q2`。在 `Q2` 中，进程通常会以轮转或先来先服务的方式运行，但只有在 `Q0` 和 `Q1` 都为空时才有机会执行。

这种调度策略背后的基本原则是：**优先处理非 CPU 密集型任务**。因为这些任务（如交互式应用）通常需要快速响应，它们在偶尔需要 CPU 时应该能被立即满足。通过将它们保持在高优先级队列，系统确保了良好的用户体验。而对于那些需要长时间计算的 CPU 密集型任务，将它们降级到低优先级队列可以防止它们长时间占用 CPU，从而影响到交互式任务的响应。当然，这种基于历史行为的猜测并不总是准确的。

多级反馈队列调度方案的一个显著特点是其高度的可配置性，这允许其具有高度的可变性，但是代价是复杂的实现，为特定的系统找到最优的参数配置也非常困难。此外，动态调整进程优先级和在队列之间移动进程也需要相当大的计算开销。为了应对这些挑战，像 Linux 内核这样的实际操作系统实现中，开发者们往往会采用一些巧妙的技巧来简化和加速这一过程。
...
异构多处理/Heterogeneous Multiprocessing/HMP 是一种多处理器架构，其中处理器核心虽然运行相同的指令集，但在时钟速度和功耗管理方面有所不同。这种设计通过根据任务的具体需求将其分配给最合适的处理器核心，来实现降低功耗的目的。

一个典型的例子是 ARM 的 **big.LITTLE** 架构，它将高性能的 `big` 核心与高能效的 `LITTLE` 核心结合在一起。`big` 核心功耗较大，适合处理需要强大计算能力的短期任务，例如交互式应用。而 `LITTLE` 核心功耗较小，适合运行那些不需要高性能但可能需要长时间运行的任务，例如后台任务。当出于省电模式的时候，操作系统也可以通过禁用高能耗的大核的方式节省电能。
