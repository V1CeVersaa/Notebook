...
- 保留站状态表：记录发射出来的每条指令的状态，一共有八个字段：
    ...
    - `Vj`、`Vk`：源操作数的值，对于每一个操作数，`Q` 和 `V` 只有一个是有效的，对于加载指令，这个字段用于保存偏移量字段；
...
实现推测背后的关键思想是允许指令乱序执行，但是强制他们**顺序提交**，并且防止指令在提交之前采取任何不可撤销的动作，比如更新状态或者引发异常，这样就可以**保证精确异常**。也可以很简单地使用在整数指令上，但是可以见得确实增加了很多的硬件复杂性。
