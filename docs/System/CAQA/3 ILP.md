...
3.1 Tomasulo’s Approach（结构图进阶）
	•	结构图详细版1：
	•	展示某一步骤下各寄存器、保留站、CDB、功能单元间的连接和数据流动。
	•	图中特别用颜色和箭头区分了指令发射、结果写回、状态更新等过程。
	•	寄存器状态表（Register status Qi）用于记录哪个保留站会最终写回某个寄存器。

3.2 Tomasulo’s Approach（结构图进阶2）
	•	结构图详细版2：
	•	展示不同阶段，寄存器和保留站的分配和使用状况。
	•	红色箭头突出显示结果如何广播到相关寄存器和保留站，保证所有依赖都能获知最新值。

3.3 Tomasulo’s Approach（结构图进阶3）
	•	结构图详细版3：
	•	更复杂的依赖场景（多个保留站同时等待同一数据），通过 CDB 广播和寄存器状态表同步数据。
	•	展示乱序调度下依赖管理的硬件协同流程。

3.4 Tomasulo’s Approach（核心三表）
	•	Tomasulo 算法核心三大表：
	1.	Instruction status table：记录每条指令的各阶段状态，仅用于算法理解，不一定硬件实现。
	2.	Reservation stations table：记录每个保留站的状态，决定操作数、功能单元等资源的分配。
	3.	Register status table (Qi 字段)：记录每个寄存器由哪个保留站负责写回，保证写回不冲突，实现寄存器重命名。

⸻

总结

整组 PPT 详细展示了动态调度、Tomasulo 算法消除数据冒险的思想和实现，从理论依赖到实际硬件结构、再到每一步的数据流动和表项管理，帮助你深刻理解硬件级乱序执行和寄存器重命名的本质。如果你想进一步看具体例子、指令执行流程或保留站状态演示，也可以告诉我！ -->
